...
class foggy_decision_tree:
    ...
    '''
    def __init__(self, max_depth=4, var=0.1):
...
if __name__ == "__main__":
    ...
    # tree = foggy_decision_tree(max_depth=2, var=0.1).fit(X_train,y_train)
    ...
    t0 = time.time()
    sklear_tree = DecisionTreeClassifier(max_depth=2, random_state=2016).fit(X_train, y_train)
    y_pred = sklear_tree.predict(X_test)
    print("Time taken: %0.3f" %(time.time() - t0))
